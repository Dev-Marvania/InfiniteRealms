...
- **Game Engine:** Primary: AI-powered via `server/gameAI.ts` using GPT-5 through Replit AI Integrations (no personal API key needed). Generates creative narrative text, mood, stat changes, and items as structured JSON. Fallback: `lib/gameEngine.ts` local engine with randomized templates used when AI is unavailable. Frontend calls `POST /api/game/command` with game state context. Supports intents: move, attack, hack, magic, rest, search.
