...
- **2026-02-13:** Added AI-generated scene images for all 6 location types (recycle, firewall, trap, neon, source, terminal) in `assets/images/scenes/`. SceneReveal component (`components/SceneReveal.tsx`) shows a full-screen animated image overlay when entering a new tile — fade+scale animation in, auto-dismisses after ~2.8s with fade out. Scene-enter sound effect plays on reveal. Each tile only reveals once per game session (tracked via ref). `getNodeType()` exported from SceneReveal maps coordinates to location types using same logic as WorldMap.
- **2026-02-14:** Combat & balance overhaul: Enemies now counterattack using their actual damage stat on every player attack. Attacks cost 5 energy, magic enforced -15 energy client-side. Edge-of-map movement blocked with narrative feedback ("wall of static"). Fallback offline engine now includes full combat (enemy spawn, counterattack, damage tracking), trace meter changes (+8 attack, +5 search, +10 rest), lore discovery on search/magic, ambush spawning on movement, Hunter Protocol at 100% trace, death/victory checks, quest item duplicate prevention, and story progress tracking. The `processCommand` function in `lib/gameEngine.ts` accepts optional `{hasFirewallKey, hasAdminKeycard}` to prevent duplicate quest item generation.
- **2026-02-14:** Added four new gameplay systems: (1) Interactive hacking minigame (`components/HackingMinigame.tsx`) — timed hex-code matching grid triggered by Hack command, act-based difficulty (Act 1: 4 pairs/15s, Act 2: 6 pairs/12s, Act 3: 8 pairs/10s), success reduces trace by 15, failure increases trace by 20 and damages player. (2) Hidden lore discovery system (`lib/loreData.ts`, `components/LoreViewer.tsx`) — 11 data logs discoverable via Search on specific tiles, lore viewer modal with collection tracker and lock icons for undiscovered entries, lore button with count badge in tab bar. (3) Visual glitch effects (`components/GlitchOverlay.tsx`) — static noise, scanlines, RGB shift, and screen flicker that intensify below 50% and 25% HP using Reanimated. (4) Trace meter (`components/TraceMeter.tsx`) — visible progress bar showing detection level, fills with actions (attack +8, search +5, rest +10, failed hack +20), decreases on movement (-8), triggers Hunter Protocol at 100%. Trace level tracked in StoryProgress. Draggable divider between narrative/terminal zones using PanResponder.
...
- **UI Components:**
  ...
  - `TraceMeter` — Animated trace detection progress bar, fills with actions, triggers Hunter Protocol at 100%
  - `HackingMinigame` — Timed hex-code matching grid overlay, act-based difficulty scaling
  - `GlitchOverlay` — Visual glitch effects (static, scanlines, RGB shift, flicker) intensifying at low HP
  - `LoreViewer` — Modal with discoverable data log collection, lock icons for undiscovered entries
  - `SceneReveal` — Full-screen animated scene image overlay on new tile entry
